...
<%@page import="com.online_banking.example.DB_Deposit"%>
	...
	if(nat_id.length() > 0 && nat_id.equals("bad")){
		...
		if(	request.getParameter("amount") != null &&
			...
			if(amountStr.length() > 0 && email.length() > 0 && password.length() > 0){
				...
				Double latestBalance = amount + oldBalance;
				...
				try {
					...
					if(login.checkValidUser(email, password)){
						if(latestBalance != (double)-1 && oldBalance > 0){
							DB_Deposit deposit = new DB_Deposit(this.user.getNat_id());
							deposit.updateDatabase(amount, oldBalance, latestBalance);
							out.println("<script>alert('The Deposit is already done successfully...');</script>");
						}else{
